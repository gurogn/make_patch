...
create_patch_dir()
{
	...
	PATCH_FOLDER=~//new_old_patch//$curDateTime
	mkdir "$PATCH_FOLDER"
}
ARCHIVE=n
DIFF=y
...
if [ $CID_NUM == 0 ] ; then 
	...
	archive_files=$(git diff --name-only HEAD^)
	if [[ -z $archive_files ]]
	then 	
	echo "The HEAD maybe a tag, there is no diff files, please try another CID"
	exit 1
	fi
	status=$(git archive -o ~/new_old_patch/new.zip HEAD $archive_files 2>&1)
	check_err "$status"	
	git archive -o ~/new_old_patch/old.zip HEAD^ $archive_files
	if [ $DIFF == "y" ] ; then
		git diff HEAD^ >> $PATCH_FOLDER/$curDateTime.diff
	fi 
elif [ $CID_NUM == 1 ] ; then
	echo "Generating the diff of commit id $1"
	archive_files=$(git diff --name-only $1 $1^)
	if [[ -z $archive_files ]]
	then 	
	echo "The CID maybe a tag, there is no diff files, please try another CID"
	exit 1
	fi
	status=$(git archive -o ~/new_old_patch/new.zip $1 $archive_files 2>&1)
	check_err "$status"
	git archive -o ~/new_old_patch/old.zip $1^ $archive_files 
	if [ $DIFF == "y" ] ; then
		git diff $1^ $1 >> $PATCH_FOLDER/$curDateTime.diff
	fi 
elif [ $CID_NUM == 2 ] ; then
	echo "Generating the diff of commit id $1 $2"
	archive_files=$(git diff --name-only $1 $2)
	if [[ -z $archive_files ]]
	then 	
	...
	status=$(git archive -o ~/new_old_patch/new.zip $1 $archive_files 2>&1)
	check_err "$status"
	status=$(git archive -o ~/new_old_patch/old.zip $2 $archive_files 2>&1)
	check_err "$status"
	if [ $DIFF == "y" ] ; then
